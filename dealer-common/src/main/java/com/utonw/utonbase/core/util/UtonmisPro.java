...
public class UtonmisPro {
	...
	public static void init(){
		...
		InputStream is = Thread.currentThread().getContextClassLoader().getResourceAsStream("config/thirdParty.properties");
		try {
			BufferedReader bf = new BufferedReader(new InputStreamReader(is,"UTF-8"));  
			properties.load(bf);
			bf.close();
		}catch(Exception e){
			throw new RuntimeException("thirdParty.properties文件未找到，请检查。");
		}
	}
	...
	public static String getUUID64(){
		return binaryTo64(hexToBinary("0" + UUID.randomUUID().toString().replaceAll("-", "").toUpperCase()));
	}
