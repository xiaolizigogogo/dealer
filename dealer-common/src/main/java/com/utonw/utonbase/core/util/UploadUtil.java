...
import com.utonw.utonbase.support.fastdfs.FileManager;
import com.utonw.utonbase.support.fastdfs.FileModel;
import com.utonw.utonbase.support.ftp.SftpClient;
...
//	private static final Logger logger = LogManager.getLogger();
    ...
    public static List<Map<String,String>> uploadFile(HttpServletRequest request) {
        ...
        List<Map<String,String>> fileNames = InstanceUtil.newArrayList();
...
//                        logger.error(name + "保存失败", e);
	...
	/** 上传文件处理(支持批量) */
	public static List<Map<String,String>> uploadImage(HttpServletRequest request, boolean lessen) {
		CommonsMultipartResolver multipartResolver = new CommonsMultipartResolver(
				request.getSession().getServletContext());
		List<Map<String, String>> fileNames = InstanceUtil.newArrayList();
		if (multipartResolver.isMultipart(request)) {
...
//							logger.error(name + "缩放失败", e);
	...
	public static List<Map<String,String>> uploadImageData(HttpServletRequest request) {
		List<Map<String,String>> fileNames = InstanceUtil.newArrayList();
...
//						logger.error("上传文件异常：", e);
	...
	public static List<Map<String,String>> uploadImageData(Map<String, String> paramMap) {
		List<Map<String,String>> fileNames = InstanceUtil.newArrayList();
		String pathDir = getUploadDir();
		File dir = new File(pathDir);
		if (!dir.exists()) {
		...
		for (String  param: paramMap.keySet()) {
			String key = param;
			String fileStr = paramMap.get(key);
			map = new HashMap<String, String>();
			if (fileStr != null && !"".equals(fileStr)) {
				int index = fileStr.indexOf("base64");
				if (index > 0) {
...
//						logger.error("上传文件异常：", e);
	...
	public static List<Map<String,String>> uploadImageDataQiNiuYun(Map<String, String> paramMap) {
		List<Map<String,String>> fileNames = InstanceUtil.newArrayList();
		String pathDir = getUploadDir();
		File dir = new File(pathDir);
		if (!dir.exists()) {
			...
			if (fileStr != null && !"".equals(fileStr)) {
				...
				if (index > 0) {
					try {
						fileStr = fileStr.substring(fileStr.indexOf(",") + 1);
						byte[] bb = Base64.getDecoder().decode(fileStr);
						Response response = uploadManager.put(bb, null, getUpToken());
...
//						logger.error("上传文件异常：", e);
	...
	public static String getUploadDir() {
		return PropertiesUtil.getString("upload.dir") + File.separator;
	}
