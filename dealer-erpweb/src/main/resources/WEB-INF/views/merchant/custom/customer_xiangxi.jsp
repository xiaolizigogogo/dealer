...
<body>
	<div class="tab_box clearfix" style="padding-top: 0px">
		<div class="tab-content clearfix">
			...
			<div class="from_box">
				...
				<div id="basic" class="from_con clearfix"></div>
			</div>
			<div class="from_box">
				<div class="from_tit clearfix">
					<h3>附件信息</h3>
				...
				<div class="img_list" id="attachment">
					<div class="clearfix">
						<h3 class="pull-left"
							style="font-size: 14px; margin: 0; font-weight: bold; padding-bottom: 12px;">
							车商基础资料</h3>
					</div>
			...
			<div class="from_box">
				...
				<div class="img_list" style="display: inline-block;">
					<a class="btn btn-danger" href="javascript:void(0)">其他征信</a>
				</div>
			</div>
			<div class="from_box">
				<div class="from_tit clearfix">
      ...
      <dl class="dl-horizontal col-sm-4">
        ...
        <dd>
          <select class="form-control pull-left"  name="reviewstatus">
			...
			<div class="from_box">
				<div class="from_tit clearfix">
					<h3 class="pull-left">车商贷款参数</h3>
				</div>
				<dl class="dl-horizontal col-sm-4">
					<dt>
						<i class="text-danger">*</i> 单车贷款比例
					</dt>
					<dd>
						<div class="input_group" style="max-width: 120px">
							<input maxlength="8" style="width: 100%;" 
								class="form-control valid" value="" type="text" id="input1"> <span
								class="danwei" >%</span>
						</div>
					</dd>
				</dl>
				<dl class="dl-horizontal col-sm-4">
					<dt>
						<i class="text-danger">*</i> 企业基准利率
					</dt>
					<dd>
						<div class="input_group" style="max-width: 120px">
							<input maxlength="8" style="width: 100%;"
								class="form-control valid" value="" type="text" id="input2"> <span
								class="danwei" >%</span>
						</div>
					</dd>
				</dl>
				<dl class="dl-horizontal col-sm-4">
					<dt>
						<i class="text-danger">*</i> 最高贷款额度
					</dt>
					<dd>
						<div class="input_group" style="max-width: 120px">
							<input type="text" name="remark" class="form-control" style="calc(100% - 70px);" id="input3"/> <span
								class="danwei" >万元</span>
						</div>
					</dd>
				</dl>
			</div>
			...
			<div class="from_box table_from_box" id="div_headquarters_audit"
 ...
 <div class="modal-footer" id="footer" style="display: none">
  ...
  <p class="col-sm-2 "> <a  href="javascript:void(0)" onclick = "submitLoanInfo(this)" class="btn btn-danger btn-block">提交</a> </p>
  </div> 
</body>
...
<script>
	...
	function init() {
		...
		$("[name='taskname']").val($("#applyStatus").val());
		$("#la1").html($("#applyStatus").val());
		$("#la3").html($("#indentCount").val());
		//获取经销商的所有信息
		$.ajax({
			type : 'post',
			...
			success : function(data) {
				$("#input1").val(data.InterestRates.limitpercentage);
				$("#input2").val(data.InterestRates.monthlyinterest);
				$("#input3").val(data.InterestRates.maximumloan);
				merchantinfo(data);
			},
			error : function() {
				layer.msg(" 系统错误！");
	...
	function merchantinfo(data) {
		...
		for ( var obj in data.customerMerPic) {
			attachmentHtml_car += attchmentAndcontract_new(
					data.customerMerPic[obj], '1');
		}
		for ( var obj in data.customerMerPic) {
			attachmentHtml_car_bank += attchmentAndcontract_new(
					data.customerMerPic[obj], '2');
		}
		for ( var obj in data.customerMerPic) {
			attachmentHtml_car_business += attchmentAndcontract_new(
					data.customerMerPic[obj], '3');
		}

		for ( var obj in data.customerContract) {
			contractHtml += attchmentAndcontract(data.customerContract[obj]);
		}
		if(data.customerBankInfo.length>0){
			$("#la2").html("已绑定");
		}else {
			$("#la2").html("未绑定");
		}
		...
		for ( var obj in bankcardList) {
			...
			console.log(data.customerBankInfo);
		}
		//基本信息
	...
	function attchmentAndcontract(param) {
		var html = "";
		if (param.fileName != "12" && param.fileName != "13"
				&& param.fileName != "14") {
			html += "<li>"
					+ "<div class='file_img'>"
	...
	var attachmentHtml_car_array = [ "公司简介", "营业执照复印件", "组织机构代码证复印件",
			"税负登记证复印件", "验资报告复印件", "法人及配偶身份证", "股东身份证", "实际控制人信用报告",
			"公司章程及修正案复印件", "企业信用报告", "企业盈利模式", "各高层管理人员简介", "法定代表人结婚证（离婚证）",
			"法定代表人户口本", "法人及股东相关资产证明", "现有员工身份证和联系方式", "税务登记证复印件" ];

	var attachmentHtml_car_bank_array = [ "近三年财务年报表", "近三个月财务月报和本月财报科目明细",
			"月业绩情况总汇表", "业务台账", "放款银行卡流水", "收款银行卡流水", "资产处置台账数据及该款项的回款账户及流水",
			"企业主要对公流水" ];

	var attachmentHtml_car_business_array = [ "客户案例", "主营业务操作流程", "风控管理及流程",
			"车辆评估标准", "业务合作方准入要求", "产品大纲", "场地租赁合同", "场地照片", "车库管理制度" ];
	...
	function attchmentAndcontract_new(param, type) {
		...
		if (param.fileName != "12" && param.fileName != "13"
			...
			if (state) {
				...
				html += "<li>"
						+ "<div class='file_new'>"
						+ "<p class='tools'><a href='javascript:void(0);' onclick='pictureDownload(7701)' title='保存'>"
						+ param.fileName
						+ "</a></p>"
						+ "<img data-original="+param.fileUrl+" src="+param.fileUrl+" alt="+param.fileName+" ></div>"
						+ "</li>"
	...
	function submitLoanInfo(){
		layer.confirm("你确定提交吗？", {icon: 3, title: '提示'}, function () {
			var item_obj={};
			item_obj.id=$("#applyId").val();
			item_obj.limitpercentage=$("#input1").val()
			item_obj.monthlyinterest=$("#input2").val()
			item_obj.maximumloan=$("#input3").val()

            $.ajax({
                type: 'POST',
                url: '${basePath}/admin/merchant/modifyMerchantInfo',
                data: item_obj,
                dataType:"text",
                success: function (data) {
                    if (data == 0000) {
