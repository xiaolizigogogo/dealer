    ...
    <div class="op_box_con clearfix">
      <form class="form-horizontal" id="form_first_trial">
        <div class=" col-sm-8">
          ...
          <%-- zhangheng add st--%>
          <div class="form-group">
              <div style="background-color: #c90c23;height: 30px;width: 100px;text-align:center;border-radius: 3px;margin-left: 35px">
                <a id="showRefundTable"  href="javascript:void(0)"  style="text-decoration:none;color:white;display: inline;vertical-align: sub;">还款计划</a>
              </div>

              <div id="refundTable" style="padding-top: 20px;display: none;margin-left: 35px">
                 <table class="table table-bordered" >
                   <thead>
                   <tr style="background-color: #eee;">
                     <th class="text-center">车行名称</th>
                     <th class="text-center">借款人</th>
                     <th class="text-center">放款时间</th>
                     <th class="text-center">放款金额(万元)</th>
                     <th class="text-center">周期(天)</th>
                     <th class="text-center">利率(%)</th>
                     <th class="text-center">还款时间</th>
                     <th class="text-center">还款金额(元)</th>
                   </tr>
                   </thead>
                   <tbody>
                   <tr>
                     <td id="carName" name="carName"></td>
                     <td id="jkpersion" name="jkpersion"></td>
                     <td id="fktime" name="fktime"></td>
                     <td id="fkamt" name ="fkamt"></td>
                     <td id="during" name ="during"></td>
                     <td id="rata" name ="rata"></td>
                     <td id="hktime" name ="hktime"></td>
                     <td id="hkamt" name="hkamt"></td>
                   </tr>
                   </tbody>
                 </table>
              </div>
          ...
          <div class="form-group">
            ...
            <div class="col-sm-8">
              ...
              <label class="radio-inline">
                ...
                <span>不通过</span> </label>
              <label class="radio-inline">
                <input type="radio" name="approvalStatus" id="inlineRadio3" value="back_sign_contract">
                <span>重新签订合同</span> </label>
            </div>
          </div>
          <%-- zhangheng add ed--%>
          <div class="form-group">
...
<script>
    ...
    function orderinfo(data){
        ...
        $("#title").find("small").attr("data-value",data.customerInfo[0].indentNumber);
 ...
 function contact(param1,param2){
   ...
   /* function attchmentAndcontract(param){
    ...
    function submitReview(){
        ...
        var paramRefoudTable  ="&carName="+$("#carName").html()+"&jkpersion="+$("#jkpersion").html()+"&fktime="+$("#fktime").html()+
            "&fkamt="+$("#fkamt").html()+"&during="+$("#during").html()+"&rata="+$("#rata").html()+"&hktime="+
            $("#hktime").html()+"&hkamt="+$("#hkamt").html()+"&lendedTime="+$("#lended_time").val()+"&imgUrl="+$("#imgShow_WU_FILE_lended").attr("src")+"&orderNum="+$("#title").find("small").attr("data-value");
        var prompt = $("input[name='approvalStatus']:checked").next("span").html();
        //获取选中的评审结论
        var dialogMsg = "确认<font color='red'>"+prompt+"</font>吗？";
        alertMessage(dialogMsg,'submit("'+param+paramRefoudTable+'")','qs_box1');
    }
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    if(data.success){
	                    ...
	                    $("#imgShow_WU_FILE_lended").attr("class","viewer-toggle");
	                    $("#imgShow_WU_FILE_lended").attr("data-original",data.imgurl);
                    }else{
                        layer.msg("上传失败 !");
                        return false;
                    }
    ...
    $("#showRefundTable").click(function () {
     ...
     var  lended_time =$("#lended_time").val();
      ...
      $.ajax({
            ...
            data:{"task_id":task_id,"lended_time":lended_time},
            dataType:"json",
            success:function (data) {
                console.log(data);
