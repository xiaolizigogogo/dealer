					...
					<div>
						<div class="form-group">
							<div class="col-sm-8  col-sm-offset-2">
								<a href="#tab7" aria-controls="tab7" role="tab"
									data-toggle="tab" id="file_more">查看全部附件</a>
							</div>
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-2 control-label"><i
							class="text-danger">*</i> 审评结论</label>
						<div class="col-sm-8">
							<label class="radio-inline"> <input type="radio"
								name="approvalStatus" id="inlineRadio1" value="pass"
								checked="checked"> <span>通过</span>
							</label> <label class="radio-inline"> <input type="radio"
								name="approvalStatus" id="inlineRadio2" value="nopass">
							<span>不通过</span>
							</label>
							<label class="radio-inline"> <input type="radio"
								name="approvalStatus" id="inlineRadio3" value="back_evaluate_install_collateral">
								<span>重新评估</span>
							</label>
							 <label class="radio-inline"> <input type="radio"
								name="approvalStatus" id="inlineRadio4" value="back_review_evaluate">
								<span>重新审核评估</span>
							</label>
							<label class="radio-inline"> <input type="radio"
								name="approvalStatus" id="inlineRadio5" value="back_review_install_collateral">
								<span>重新审核押品</span>
							</label>
						    <label class="radio-inline"> <input type="radio"
								name="approvalStatus" id="inlineRadio6" value="back_first_trial">
								<span>重新初审</span>
							</label>
						</div>
						</div>
					<div class="form-group">
						<label class="col-sm-2 control-label">车辆审批价格</label>
						<div class="col-sm-8">
							<input class="form-control" id="carJudgeMoney" name="carJudgeMoney" style="max-width:100px" readonly/>
						</div>
					</div>
					<div class="form-group">
							<label class="col-sm-2 control-label"><i
								class="text-danger">*</i>终审贷款金额(万元)</label>
							<div class="col-sm-8">
								<input  class="form-control" type="number"
									id="judgeMoney" name="judgeMoney" style="max-width:100px; display: inline;"></input>
									<span  class="text-muted">要求不能大于车辆审批金额*80%</span>
							</div>
					</div>
		
						<div class="form-group">
							<label class="col-sm-2 control-label"><i
								class="text-danger">*</i> 意见</label>
							<div class="col-sm-8">
								<textarea cols="168" rows="2" class="form-control"
									id="approval_first_trial" name="approvalConclusion"></textarea>
							</div>
						</div>
				</form>
				<input type="hidden" id="maximumLoan">
			</div>
		</div>
	</div>
	<div class="modal-footer">
		...
		<p class="col-sm-2">
			<a href="javascript:void(0)" onclick="submitReview()"
				class="btn btn-danger btn-block" id="commit">提交处理</a>
		</p>
	</div>
</body>
...
<script>
	...
	function init() {
		...
		//CarDealerLoan._initProcessBar("", $("#loan_processBar"));
		getInitData();
	...
	function orderinfo(data) {
		...
		for (var i = 0; i < data.reviewInfo.length; i++) {
			...
			if("风控审核车辆评估" == data.reviewInfo[i].loanStatus){
				$("#carJudgeMoney").val(data.reviewInfo[i].approveMoney+"万元");
			}
		}
 ...
 function contact(param1,param2){
	...
	function submitReview() {
		...
		var carJudgeMoney=$("#carJudgeMoney").val();
		var judgeMoney=$("#judgeMoney").val();
		var maximumLoan=$("#maximumLoan").val();
		if (judgeMoney == "" || judgeMoney == null||parseFloat(judgeMoney)<=0) {
			layer.msg("审批金额错误!");
			return false;
		}
		if(parseFloat(judgeMoney)>parseFloat(carJudgeMoney)*0.8){
			layer.msg("终审金额不能大于审批金额的百分之八十!");
			return false;
		}
		if (parseFloat(judgeMoney)>parseFloat(maximumLoan)) {
			layer.msg("审批金额不能大于最高贷款额！");
			return false;
		}
		var param = "&taskId=" + $("#task_id").val() + "&orderId="
				+ $("#indent_uuid").val() + "&approvalLevel="
				+ $("#flowStatus_select").val()+"&approvalAmount="+judgeMoney;
