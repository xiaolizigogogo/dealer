...
<div class="op_box" id=" op_box_first_trial">
	...
	<dl class="dl-horizontal col-sm-12">
				<dt>状态</dt>
				<dd>      
					 <span class="text-value" data-key="loanStatus"></span>
				</dd>
			</dl>
			<dl class="dl-horizontal col-sm-12">
				<dt><i class="text-danger">*</i> 审核意见</dt>
				<dd>      
					<select class="form-control" name="endType">
						<option value="" selected="">---- 请结束订单类型 ---- </option>
						<option value="坏账订单" data-code="110000">坏账订单</option>
						<option value="线下追回" data-code="110000">线下追回</option>
					</select>
				</dd>
			</dl>
...
<div class="modal-footer">
	...
	<p class="col-sm-2">
		<a href="javascript:void(0)" onclick="submitReview()"
		   class="btn btn-danger btn-block" id="commit">结束订单</a>
	</p>
</div>
</body>
...
<script>
 ...
 function contact(param1,param2){
	...
	function submitReview() {
		debugger;
		var endType=$("select[name='endType']").val()
		if(endType==null||endType.length==0){
			layer.alert("请选择结束类型")
			return ;
		}
		var param = "&taskId=" + $("#task_id").val() + "&orderId="
				+ $("#indent_uuid").val() + "&approvalLevel="+$("#flowStatus_select").val()+"&approvalStatus=pass&endType="+endType;
		//获取选中的评审结论
		var dialogMsg = "确认<font color='red'>结束订单</font>吗？";
		alertMessage(dialogMsg, 'submit("' + param + '")', 'qs_box1');
	}
	function submit(param) {
		$.ajax({
			type : "post",
			url : "/admin/order/overdueHandler",
