...
<div class="main-con">
    <form class="form-inline clearfix" action="javascript:;" method="POST">
        <div class="normal-area">
            ...
            <div class="title clearfix"><h2>续借合同上传</h2>&nbsp;&nbsp;&nbsp;&nbsp;</div>
            <div class="con clearfix">
                <div class="row">
                    <div class="form-group col-md-6">
                        <label class="label-s">上传续借合同<span id="continueUploadSpan"
                                                           class="cred">(必传)</span></label>
                        <input id="continueUpload" type="file"
                               class="form-control"
                               name="files[]"
                               data-url="${basePath}/admin/file/upload/${loanInfo.uuid}/continueUpload.html" />
                    </div>
                </div>
            </div>
...
<script type="application/javascript">
    ...
    $(function () {
        ...
        $("#continueUpload").fileupload({
            dataType: 'json',
                ...
                doneProcess(data, $('#continueUploadSpan'));
            }
        });
    ...
    var init = function () {
        $("#loan_attachment_div").load("${basePath}/admin/loan/getUploadLoanAttachmentUnderCondition.html?loanId=${loanInfo.uuid}", function () {
    ...
    var checkAttachmentUploaded = function () {
        /**改变已上传附件的样式*/
        var continueUpload = $("#continueUpload").val();
        if (continueUpload !== undefined && continueUpload !== null && continueUpload !== "") {
            var continueUploadSpan = $("#continueUploadSpan");
            continueUploadSpan.removeClass("cblue");
            continueUploadSpan.addClass("cgreen");
            continueUploadSpan.text("已上传");
        }
    };
    ...
    function submit() {
        ...
        layer.confirm(dialogMsg, {icon: 3, title: '提示'}, function () {
            ...
            $.ajax({
                type: 'POST',
                url: '${basePath}/admin/loan/continueProceduresReview.html',
	            beforeSend: function () {
		            load = layer.load();
	            },
                data: {
