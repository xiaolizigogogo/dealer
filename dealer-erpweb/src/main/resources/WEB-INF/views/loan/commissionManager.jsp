...
<script language="javascript">
    ...
    function search() {
        ...
        var url = "${basePath}/admin/financeManager/commissionManager?conditionKey=" + conditionKey + "&conditionValue=" + conditionValue
                + "&isEnded=" + isEnded + "&startDate=" + chargedoffAt + "&endDate=" + chargedoffAtEnd;
...
<div class="main-con">
    ...
    <div class="table-area" id="con_one_1">
        <table border="0" cellspacing="0" cellpadding="0"
            ...
            <c:forEach items="${resultComList}" var="l">
                <tr>
                    <td align="center">${l.customerName}</td>
                    <td align="center">${l.loanMoney }</td>
                    <td align="center">${l.captialSum}</td>
                    <td align="center">${l.interestSum}</td>
                    <td align="center">${l.repayPeriod}</td>
                    <td align="center">${l.openId}</td>
                    <td align="center">${l.orderId}</td>
                    <td align="center">${l.nickname}</td>
                    <td align="center">${l.idCard}</td>
                    <td align="center">${l.userCommission}</td>
                    <td align="center">${l.openIdComission}</td>
                    <td align="center">${l.commissionRate}</td>
                    <td align="center">${l.openIdrate}</td>

                    <c:choose>
                        <c:when test="${l.isEnded == 1}">
                            <td align="center">结清</td>
                        </c:when>
                        <c:when test="${l.isEnded == 0}">
                            <td align="center">未结清</td>
                        </c:when>
                    </c:choose>

                    <td align="center"><fmt:formatDate value="${l.repayTime}"
                                                       pattern="yyyy-MM-dd HH:mm"/></td>
                    <td align="center"><fmt:formatDate value="${l.chargedoffAt}"
                                                       pattern="yyyy-MM-dd HH:mm"/></td>

                    <td align="center">
                        <a href="javascript:updateRate('${l.userId}','${l.customerId}')"
                           class="link-01">修改提成比例</a>
                    </td>
                </tr>
            </c:forEach>
            <tr>
                <td colspan="17">
                    共有<strong style="color:red">${pageSize}</strong>条记录,分为<strong
                        style="color:red">${totalpage}</strong>页,当前是<strong
                        style="color:red">${cpage+1}</strong>页
                    &nbsp; &nbsp;
                    <c:if test="${haslast }">
                        <a href="javascript:goPage(0)">首页</a>
                        <a href="javascript:goPage(${cpage-1 })">上一页</a>
                    </c:if>
                    <c:if test="${!haslast }">
                        首页
                        上一页
                    ...
                    <c:if test="${hasnext }">
                        <a href="javascript:goPage(${cpage+1})">下一页</a>
                        <a href="javascript:goPage(${totalpage-1})">尾页</a>
                    </c:if>
                    <c:if test="${!hasnext }">
                        下一页
                        尾页
