...
<script type="text/javascript">
		function  reset1(){
			...
			$.ajax({
				type:"post",
				url:"/admin/mail/tesetpassword.html?uuid=" + $("#uuid").val()+"&date="+$("#date").val()+"&newpass="+newpass,
				success:function(data){
					
					if(data.a=="yes"){
						alertMessage("修改密码成功请重新登录","login1()");
					}else if(data.a=="overtime"){
						alertMessage("已超时请重新发送邮件","again()");
					}
					else{
...
<div class="container">
  <div class="col-lg-6 col-lg-offset-3 clearfix" id="login_box">
    ...
    <div class="form-group">
      ...
      <div class="clearfix">
        <button type="button" class="btn btn-danger col-sm-3"  onclick="reset1()" >重置密码</button>
      </div>
    </form>
  </div>
</div>
