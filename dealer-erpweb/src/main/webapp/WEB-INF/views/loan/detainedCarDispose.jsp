                ...
                <form class="form-inline clearfix">
                    ...
                    <div class="row">
                        <div class="form-group col-md-6">
                            <label class="label-s">收款凭证上传<span
                                    id="repaymentUploadProofUploadSpan" class="small_span" >
                                        (上传的附件在借款附件栏目查看) </span><span id="idCardFileuploadSpan"
                                                                      class="cred">(必传)</span></label>
                            <input id="repaymentUploadProofUpload" type="file"
                                   class="form-control"
                                   name="files[]"
                                   data-url="${basePath}/admin/file/upload/${repayment.uuid}/ownerVehicleRepayment.html"/>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div>
                            ...
                            <button type="button" class="btn btn-01" name="submit1" id="submit1"
                                    onclick="javascript:detainedCarDispose('saleRepayment')">已卖出
                            </button>
                            &nbsp; &nbsp; &nbsp;
                            <button type="button" class="btn btn-01" name="submit3" id="submit4"
                                    onclick="javascript:detainedCarDispose('detainedCarRepayment')">客户还款提车
                            </button>
                            &nbsp; &nbsp; &nbsp;
                            <button type="button" class="btn btn-01" name="submit3" id="submit5"
                                    onclick="javascript:detainedCarDispose('vehicleNoSaleRepayment')">还款不提车
                            </button>
...
<div id="dialog-confirm" title="输入卖出金额 " style="display: none" >
    <p>
        <span> 卖出金额:<input name="saleMoney" id="saleMoney" type="text"></span>
    </p>
</div>



<div id="dialog-confirm2" title="输入提车还款金额 " style="display: none" >
    <p>
        <span> 还款金额:<input name="repaymentMoney" id="repaymentMoney" type="text"></span>
        <input name="guranteeInfoId" id="guranteeInfoId" type="hidden" value="${loanInfo.guaranteeEvaluateInfo.id}"/>
        <input name="guranteeInfoLoanId" id="guranteeInfoLoanId" type="hidden" value="${loanInfo.guaranteeEvaluateInfo.loanId}"/>
    </p>
</div>

<div id="dialog-confirm3" title="输入还款金额" style="display: none" >
    <p>
        <span> 还款金额:<input name="repaymentMoney" id="repaymentMoney2" type="text"></span>
        <input name="guranteeInfoId" id="guranteeInfoId2" type="hidden" value="${loanInfo.guaranteeEvaluateInfo.id}"/>
        <input name="guranteeInfoLoanId" id="guranteeInfoLoanId2" type="hidden" value="${loanInfo.guaranteeEvaluateInfo.loanId}"/>
    </p>
</div>
<script>
    ...
    function detainedCarDispose(flag) {
        ...
        $.ajax({
            type: 'POST',
            url: '${basePath}/admin/loan/updateRepaymentStatus',
            data: {
                "taskId": taskId,
                "repaymentFlag": flag
            },
     ...
     *  已拖车处理
     */
    function CarDispose(flag) {

        var saleMoney = $("#saleMoney").val();
        var guranteeInfoId = $("#guranteeInfoId").val();
        var guranteeInfoLoanId = $("#guranteeInfoLoanId").val();
        var repaymentMoney = $("#repaymentMoney").val();
        var repaymentMoney2 = $("#repaymentMoney2").val();
        var guranteeInfoId2 = $("#guranteeInfoId2").val();
        var guranteeInfoLoanId2 = $("#guranteeInfoLoanId2").val();
        var taskId = "${taskId}";
        ...
        if (flag == "saleRepayment") {
            ...
            guranteeInfoId = guranteeInfoId2;
            guranteeInfoLoanId = guranteeInfoLoanId2;
        }

            $.ajax({
                type: 'POST',
                url: '${basePath}/admin/loan/completeDetainedCarDispose',
                data: {
                    "saleMoney": saleMoney,
        ...
        $("#dialog-confirm2").dialog(
                {
                    ...
                    buttons: {
                        "确认": function() {
                            CarDispose("detainedCarRepayment")
                            $(this).dialog('close');
                        },
                        "取消": function() {
                            $(this).dialog('close');
        ...
        $("#dialog-confirm3").dialog(
                {
                    ...
                    buttons: {
                        "确认": function() {
                            CarDispose("vehicleNoSaleRepayment")
                            $(this).dialog('close');
                        },
                        "取消": function() {
                            $(this).dialog('close');
